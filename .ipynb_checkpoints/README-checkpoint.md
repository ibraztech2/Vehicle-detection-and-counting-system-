...
High-Resolution Camera ( for online feed)
...
Python 2.9 or 3.x
 Libraries:
    ...
    - Bytetrack
    - Numpy
    - Torch version 1.7 or older
    - Ultralytics
    - Opencv_python
    - Loguru
    - Torchvision verion 0.10.0 or older
    - Pillow
    - ninja
    - Lap
    - cython-bbox
...
`Note`:
- To run this on your local machine the listed software reuirement must be met.
- This can be done using 'pip install -r requirement.txt'
- To run inference locally
   python main.py --(path to the video if offline) if not provided the script download a default video from google drive.
